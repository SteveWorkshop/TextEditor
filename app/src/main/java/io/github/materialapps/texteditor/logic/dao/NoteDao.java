...
public interface NoteDao {
    ...
    @SuppressWarnings(RoomWarnings.CURSOR_MISMATCH)
    @Query("select Note.id as id,Note.title,Note.absc as absc,Note.createTime,Note.updateTime,Tag.id as tagId,Tag.tagName,Tag.color from Note left join Tag on Note.tag=Tag.id order by Note.updateTime desc")
    DataSource.Factory<Integer,NoteVO> getPreviewByPage();

    @Query("select Note.id as id,Note.title,Note.content,Note.createTime,Note.updateTime,Tag.id as tagId,Tag.tagName,Tag.color from Note left join Tag on Note.tag=Tag.id where Note.id=:id")
